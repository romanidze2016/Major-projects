#include "gamedialog.h"
#include "bullet.h"
#include "ship.h"
#include <QDebug>
#include <QKeyEvent>
#include <QMouseEvent>
#include <QPainter>
#include <QPixmap>
#include <QSound>
#include <QTimer>
#include <QWidget>
#include <vector>

namespace game {

GameDialog::GameDialog(QWidget* parent)
        : QDialog(parent), bullets(), shipFiringSound(this), gameScore(0), level(1), gameOver(false) {
    initializeGame();
    setMouseTracking(true);
    update();
}

void GameDialog::initializeGame() {
    // SET UP GAME DIMENSIONS AND CONFIG
    Config* c = Config::getInstance();
    SCALEDWIDTH = c->get_SCALEDWIDTH();
    SCALEDHEIGHT = c->get_SCALEDHEIGHT();
    if (frames == 0) {
        this->frames = c->get_frames();
    }
    frameCounter = 1;

    // EXTENSION STAGE 1 PART 1 - RESCALE GAME SCREEN FOR SHIP SIZE
    this->setFixedWidth(SCALEDWIDTH);
    this->setFixedHeight(SCALEDHEIGHT);

    // MENU
    QList<QPair<QString, int>> dummy;
    menu = new Menu(this, c->get_name(), this->gameScore, this->level, this->frames, this->controls, dummy);
    if (!c->getLevelType().startsWith("default")) {
        menu->showLevel();
        levelLabelCounter = 1;
    }

    // SHIP
    QPixmap ship;
    ship.load(":/Images/ship.png");
    this->ship = new Ship(ship, c->get_scale(), c->get_startpos(), SCALEDHEIGHT);
    this->next_instruct = 0;
    // SHIP SOUND
    shipFiringSound.setSource(QUrl::fromLocalFile(":/Sounds/shoot.wav"));
    shipFiringSound.setVolume(0.3f);

    // ALIENS
    generateAliens(c->getSwarmList());

    //Count the total number of aliens
    initialNumOfAliens = 0;
    for (AlienBase* s: swarms->getAliens()) {
        initialNumOfAliens += s->getAliens().size();
    }
    initialNumOfAliens--;
    aliensLeft = initialNumOfAliens;

    // SET BACKGROUND
    setStyleSheet("background-color: #000000;");

    //Reinitialize timer
    paused = false;
    timer = new QTimer(this);
    connect(timer, SIGNAL(timeout()), this, SLOT(nextFrame()));
    timer->start(this->frames);

    //set lives to zero if the old config is loaded
    if (c->getLevelType().startsWith("default")) {
        livesLeft = 1;
    }
}

void GameDialog::resetGame() {
    Config* c = Config::getInstance();
    if (c->getLevelType().startsWith("default") || c->getLevelType().startsWith("last")) {
        return;
    }

    delete ship;
    delete timer;  // optional, don't have to do this apparently

    // loop though swarms to delete aliens
    delete swarms;  // recursively deletes itself.
    for (Bullet* b : bullets) {
        delete b;
    }
    bullets.clear();

    if (box != NULL) {
        delete box;
        box = NULL;
    }

    level++;
    QString fileName = "../SpaceInvaders/level" + QString::number(level) + ".txt";
    c->loadNewSettings(fileName);

    initializeGame();
}

GameDialog::~GameDialog() {
    delete ship;
    delete timer;  // optional, don't have to do this apparently
    delete menu;

    // loop though swarms to delete aliens
    delete swarms;  // recursively deletes itself.
    for (Bullet* b : bullets) {
        delete b;
    }

    if (box != NULL) {
        delete box;
    }
}

// make the swarms for this level.
void GameDialog::generateAliens(const QList<SwarmInfo>& makeSwarms) {
    SwarmInfo def = SwarmInfo();

    swarms = new Swarm(def, *this->ship);
    for (SwarmInfo info : makeSwarms) {
        AlienBase* s = new Swarm(info, *this->ship);
        swarms->add(s);
    }
}

void GameDialog::pauseStart() {
    if (this->paused) {
        // start game
        this->paused = false;
        this->menu->displayMenu(paused);
        this->timer->start(frames);
    } else {
        this->paused = true;
        this->menu->displayMenu(paused);
        this->timer->stop();
    }
    this->setFocusPolicy(Qt::StrongFocus);
}

void GameDialog::keyPressEvent(QKeyEvent* event) {
    Config* c = Config::getInstance();
    if (event->key() == Qt::Key_P) {
        pauseStart();
    }
    else if (!this->paused && controls.startsWith("keyboard") && !c->getLevelType().startsWith("default")){
            if (event->key() == Qt::Key_Left) {
                ship->move_left();
            }
            else if (event->key() == Qt::Key_Right) {
                ship->move_right();
            }
            else if (event->key() == Qt::Key_Space) {
                //bullets.push_back(this->ship->shoot());
                addBullets(this->ship->shoot());
                this->shipFiringSound.play();
            }
        }
}

void GameDialog::mouseMoveEvent(QMouseEvent *event){
    Config* c = Config::getInstance();
    if (!this->paused && controls.startsWith("mouse") && !c->getLevelType().startsWith("default")) {
        ship->set_x(event->pos().x() - ship->get_ship_image().width()/2);
    }
}

void GameDialog::mousePressEvent(QMouseEvent *event) {
    Config* c = Config::getInstance();
    if(event->button() == Qt::LeftButton && !this->paused && controls.startsWith("mouse") && !c->getLevelType().startsWith("default")) {
        addBullets(this->ship->shoot());
        this->shipFiringSound.play();
    }
}

// shows this game score
void GameDialog::showScore() {
    // in future, implement 'score list' in menu.
    menu->openScore();
    this->setFocusPolicy(Qt::StrongFocus);
}

//Sets controls to mouse
void GameDialog::setMouse() {
    controls = "mouse";
    this->setFocusPolicy(Qt::StrongFocus);
    menu->highlightControl();
}

//Sets controls to keyboard
void GameDialog::setKeyboard() {
    controls = "keyboard";
    this->setFocusPolicy(Qt::StrongFocus);
    menu->highlightControl();
}

void GameDialog::hideMenu() {
    this->paused = false;
    this->menu->displayMenu(paused);
    this->timer->start(frames);
    this->setFocusPolicy(Qt::StrongFocus);
}

//Displays controls settings
void GameDialog::showControls() {
    menu->displayControls();
    this->setFocusPolicy(Qt::StrongFocus);
}

//Displays game speed settings
void GameDialog::showSpeeds() {
    menu->displaySpeeds();
    this->setFocusPolicy(Qt::StrongFocus);
}

//Sets the game speed to low
void GameDialog::setLowSpeed() {
    frames = 60;
    menu->highlightSpeed();
    this->setFocusPolicy(Qt::StrongFocus);
}

//Sets the game speed to medium
void GameDialog::setMediumSpeed() {
    frames = 45;
    menu->highlightSpeed();
    this->setFocusPolicy(Qt::StrongFocus);
}

//Sets the game speed to high
void GameDialog::setHighSpeed() {
    frames = 30;
    menu->highlightSpeed();
    this->setFocusPolicy(Qt::StrongFocus);
}

// FOLLOWING EACH INSTRUCTION TO FRAME - for PLAYER ship.
void GameDialog::nextFrame() {

    if (!paused) {
        //hide level label if the time has come
        if (levelLabelCounter > 40) {
            levelLabelCounter = 0;
            menu->hideLevel();
        }
        else if (levelLabelCounter > 0) {
            levelLabelCounter++;
        }

        Config* c = Config::getInstance();

        QStringList instruct = c->get_instructs();
        if (next_instruct >= instruct.size()) {
            next_instruct = next_instruct % instruct.size();
        }
        QString ins = instruct[next_instruct];
        next_instruct++;

        if (ins == "Left") {
            ship->move_left();
        } else if (ins == "Right") {
            ship->move_right();
        } else if ((instruct.size() != 1 || c->getLevelType().startsWith("default")) && ins == "Shoot") {
            addBullets(this->ship->shoot());
            this->shipFiringSound.play();
        }

        updateBullets();

        // loop through each alien swarm, move and calculated collisions
        int ratio = ((double) aliensLeft/(double) initialNumOfAliens)*10.0;
        if (ratio == 0 ||   frameCounter%ratio == 0 || c->getLevelType().startsWith("default")) {
            swarms->move("");  // recursive.
            checkSwarmCollisions(swarms);
            addBullets(swarms->shoot(""));
            frameCounter = 1;
        }
        else {
            frameCounter++;
        }

        //drop the super power box if more than 1/2 of the aliens have been destroyed
        if (ratio <= 5 && box == NULL && !c->getLevelType().startsWith("default")) {
            QPixmap boxImg;
            boxImg.load(":/Images/box.png");
            boxImg = boxImg.scaledToHeight(45);
            box = new Base(boxImg, 0, (int) (rand() % (c->get_SCALEDWIDTH() - boxImg.width())), 0, c->get_SCALEDWIDTH() - boxImg.width(), c->get_SCALEDHEIGHT() + 20);
        }
        updateBox();

        //if the super power has been on for too long => change the spaceshipt's state
        if (superPowerCounter > 100) {
            ship->setNormalState();
            superPowerCounter = 0;
        }
        else if (superPowerCounter > 0) {
            superPowerCounter++;
        }

        //all aliens are destroyed => update the level
        if (swarms->getAliens().size() == 0) {
            resetGame();
        }
    }
    // prepare collisions and calculate score
    update();
}

void GameDialog::updateBox() {
    Config* c = Config::getInstance();
    int height = c->get_SCALEDHEIGHT();

    if (box == NULL || box->get_y() > height) {
        return;
    }

    box->set_y(box->get_y() + 5);
    if (box->collides(*this->ship)) {
        box->set_y(height + 10);
        ship->setSuperState();
        superPowerCounter = 1;
    }
}

void GameDialog::paintBullets(QPainter& painter) {
    for (int i = 0; i < bullets.size(); i++) {
        Bullet* b = bullets[i];
        painter.drawPixmap(b->get_x(), b->get_y(), b->get_image());
    }
}

void GameDialog::updateBullets()
{
    for (int i = 0; i < bullets.size(); i++) {
        Bullet* b = bullets[i];
        // WHEN BULLET OFF GAME SCREEN, FREE MEMORY AND DELETE
        int score = get_collided(b, swarms);
        if (b->get_y() < 0 || b->get_y() >= SCALEDHEIGHT || b->get_x() < 0 ||
                b->get_x() >= SCALEDWIDTH || score > 0) {
            delete b;
            bullets.erase(bullets.begin() + i);
            i--;
        } else if (score == -1) {
            // DEAD SHIP!
            if (livesLeft <= 1) {
                close();
            }
            else {
                livesLeft--;
                bullets.erase(bullets.begin() + i);
            }
        } else
        {
            b->move();// we move at the end so that we can see collisions before the game ends
        }
        gameScore += score;


    }
}

// recurse throughout the tree and draw everything.
// will also
void GameDialog::paintSwarm(QPainter& painter, AlienBase*& root) {
    for (AlienBase* child : root->getAliens()) {
        // if the child is a leaf (i.e., an alien that has no children), draw it.
        const QList<AlienBase*>& list = child->getAliens();
        if (list.size() == 0) {  // leaf
            painter.drawPixmap(child->get_x(), child->get_y(), child->get_image());
        } else {
            paintSwarm(painter, child);
        }
    }
}

//check if any aliens based off the alien root are crashing with the player ship.
void GameDialog::checkSwarmCollisions(AlienBase *&root)
{
    for (AlienBase* child : root->getAliens()) {
        // if the child is a leaf (i.e., an alien that has no children), check for collisions.
        const QList<AlienBase*>& list = child->getAliens();
        if (list.size() == 0) {  // leaf
            // check if it is crashing into the player ship
            if (child->collides(*this->ship)) {
                close();// DEAD SHIP AGAIN
            }
        } else {
          checkSwarmCollisions(child);
        }
    }
}

// PAINTING THE SHIP AND ANY BULLETS
void GameDialog::paintEvent(QPaintEvent*) {
    // SHIP
    QPainter painter(this);

    painter.drawPixmap(ship->get_x(), ship->get_y(), ship->get_ship_image());

    // loop through each alien swarm and draw
    paintSwarm(painter, swarms);

    // BULLETS last so they draw over everything
    paintBullets(painter);

    //BOX WITH SUPER POWERS
    Config* c = Config::getInstance();
    if (box != NULL && box->get_y() <= c->get_SCALEDHEIGHT()) {
        painter.drawPixmap(box->get_x(), box->get_y(), box->get_image());
    }

    //if the old config is loaded don't display lives
    if (c->getLevelType().startsWith("default")) {
        return;
    }

    //Lives left
    QPixmap heart;
    heart.load(":/Images/heart.png");
    heart = heart.scaledToHeight(30);
    int currentX = SCALEDWIDTH - heart.width() - 5;
    for (int i = 0; i < livesLeft; i++) {
        painter.drawPixmap(currentX, 5, heart);
        currentX -= heart.width() - 5;
    }
}

// if this bullet is unfriendly, only check if it hits Ship
// if this bullet is friendly, will check the swarm;
// returns the score from the deleted hit object.
// Returns 0 if nothing hit, -ve if ship is hit.
int GameDialog::get_collided(Bullet*& b, AlienBase*& root) {
    int totalScore = 0;

    // if it's an enemy bullet, then don't look at the swarm.
    if (!b->isFriendly()) {
        // check it hits the player ship
        if (b->collides(*this->ship)) {
            totalScore = -1;

        }  // future; add barriers here.
    } else {
        totalScore += get_collided_swarm(b, root);
    }
    return totalScore;
}

// helper function, recursively searches swarms.
int GameDialog::get_collided_swarm(Bullet*& b, AlienBase*& root) {
    int totalScore = 0;
    // Case 1: you are a leaf node, or root node
    if (root->getAliens().size() == 0) {
        // check collision.
        if (b->collides(*root)) {
            // if it is an alien, it will return >0.
            // if it's a swarm (i.e., root) it will return 0.
            aliensLeft--;
            return root->get_score();
        }

    } else {
        for (int i = 0; i < root->getAliens().size(); i++) {
            AlienBase* child = root->getAliens().at(i);
            // if it's another swarm, recurse down the tree
            totalScore += get_collided_swarm(b, child);
            // if something was hit, score > 0
            if (totalScore > 0 && child->getAliens().size() == 0) {
                // some children shoot more bullets when they die.
                // ask child for reaction when you're going to delete them
                addBullets(child->react());
                root->remove(child);
                i--;
                return totalScore;
            }
        }
    }
    return totalScore;
}

void GameDialog::addBullets(const QList<Bullet*>& list) {
    for (Bullet* b : list) {
        this->bullets.push_back(b);
    }
}
}
