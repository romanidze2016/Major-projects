...
Config::Config() {
    ...
    QFile c_file("../SpaceInvaders/level1.txt");
...
void Config::loadNewSettings(QString fileName) {
    // GENERAL READING OF CONFIG FILE
    swarmList = {};
    QFile c_file(fileName);
    c_file.open(QIODevice::ReadOnly);
    ...
    while (!in.atEnd()) {
        ...
        if (l.isEmpty()) {
            ...
            if (!in.atEnd()) {
                ...
                if (l.startsWith("intermidiate") || l.startsWith("last")) {
                    levelType = l;
                }
            }
        } else {
